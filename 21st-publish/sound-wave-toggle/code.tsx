...
export type SoundWaveToggleProps = {
  /** Public URL to looped audio (e.g. CDN or `/sample.mp3` in your app `public/`) */
  audioSrc: string;
  className?: string;
 ...
 * Tries autoplay on mount; if blocked, the user can tap to unmute.
 * Optional: `window.dispatchEvent(new Event("hyperia:start-sound"))` to retry autoplay.
 *
 * No npm dependencies beyond React (fades use requestAnimationFrame).
 */
export function SoundWaveToggle({
  ...
  return (
    <div className={cx("inline-flex", className)}>
      ...
      >
        {showLabel ? (
          <span id={labelId} className={cx("select-none", labelClassName)}>
            Sound
          </span>
        ) : null}
        <canvas
...
export default SoundWaveToggle;
